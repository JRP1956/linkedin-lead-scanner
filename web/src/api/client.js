...
export async function markFollowUpReplied(id) {
  const res = await fetch(`${API_BASE}/follow-ups/${id}/replied`, { method: 'POST' });
  if (!res.ok) throw new Error((await res.json()).message || 'Failed to mark replied');
  return res.json();
}
